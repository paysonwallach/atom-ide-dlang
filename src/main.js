...
class DLSLanguageClient extends AutoLanguageClient {
  ...
  preInitialization(connection) {
...

    {
      let busyMessage
      connection.onCustom(
        "$/dls.upgradeSelections.start",
        (params) => (busyMessage = busyService.reportBusy(params.tr))
      )
  ...
  filterChangeWatchedFiles(filePath) {
    return (
      ["dub.json", "dub.sdl", "dub.selections.json", ".gitmodules"].indexOf(path.basename(filePath)) !== -1 ||
  ...
  startServerProcess() {
    ...
    return busyServicePromise.then(() =>
      busyService.reportBusyWhile("Installing DLS", () => promise, {
        revealTooltip: true,
      })
    )
  }
...
function getDlsPath() {
  ...
  let dlsDir = path.join(
    process.env[util.isWindows ? "LOCALAPPDATA" : "HOME"],
